        ...
        if (transactionId) {
            axios
                ...
                .then((response) => {
                    ...
                    console.log("📥 โหลดข้อมูลธุรกรรมเดิม:", data);

                    setAmount(
                        data.amount ? Math.abs(data.amount).toString() : ""
                    ); // ✅ แปลงเป็น string
                    setNote(data.description || ""); // ✅ ค่าดีฟอลต์ถ้าไม่มีรายละเอียด
                    setTransactionType(
                        data.transaction_type === "income"
                            ? "income"
                            : "expense"
                    ); // ✅ ตรวจสอบประเภท
                    setCategory(data.category_id || categories[0]?.id); // ✅ ถ้าหมวดหมู่หาย ให้ใช้ค่าเริ่มต้น
                })
                .catch((error) => {
                    console.error("❌ โหลดข้อมูลธุรกรรมไม่สำเร็จ:", error);
                });
        }
    }, [transactionId]);
